...
from tictactoe.forms import InvitationForm, MoveForm
from tictactoe.models import Invitation, Game, Move
...
def accept_invitation(request, pk):
    ...
    if request.method == 'POST':
        ...
        return render(request, "tictactoe/accpet_invitation.html", {
...
def game_detail(request, pk):
    ...
    if game.is_user_move(request.user):
        return redirect('game_make_move', pk=pk)
    return render(request, 'tictactoe/game_detail.html', {'game': game})


@login_required
def game_make_move(request, pk):
    game = get_object_or_404(Game, pk=pk)
    ...
    return render(request, 'tictactoe/game_make_move.html', context)
...
class AllGamesView(ListView):
    ...
    def dispatch(self, *args, **kwargs):
        return super(AllGamesView, self).dispatch(*args, **kwargs)